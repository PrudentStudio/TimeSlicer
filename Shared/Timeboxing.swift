...
func createTimeboxes(startDate: Date, endDate: Date, time_interval: Int = 10) -> [Timebox] {
    ...
    func isWithinWorkingHours(start: Date, end: Date) -> Bool {
        ...
        let userDefinedStartComponents = calendar.dateComponents([.hour, .minute], from: UserDefaults(suiteName: "group.com.navanchauhan.timeslicer")!.object(forKey: "DayStart") as? Date ?? Calendar.current.date(from: DateComponents(hour: 8))!)
        let userDefinedEndComponents = calendar.dateComponents([.hour, .minute], from: UserDefaults(suiteName: "group.com.navanchauhan.timeslicer")!.object(forKey: "DayEnd") as? Date ?? Calendar.current.date(from: DateComponents(hour: 20))!)
